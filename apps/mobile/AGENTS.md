...
- Pure functions must be deterministic and must not perform network or device-storage I/O, read ambient time or randomness, mutate shared state, log, or depend on framework lifecycle state.
- Keep effects explicit at narrow boundaries such as API clients, persistence adapters, hooks, event handlers, notifications, analytics adapters, and navigation or application shells.
- Keep screens, hooks, and event orchestrators thin: compose pure functions and explicit effects rather than owning duplicated business rules.
...
Organize mobile code by cohesive repository/domain before global technical categories.

- A domain repository owns its model/types, CRUD and API methods, validation, domain-specific selectors and utilities, state logic, feature screens/components, and tests when they belong only to that domain.
- Keep project-specific UI, API access, state transitions, and utilities together under the project domain area rather than scattering them across global screens, hooks, services, and utils directories.
...
- Keep network, device storage, timers, subscriptions, notifications, analytics, navigation, and framework lifecycle effects outside pure domain functions and make their boundaries obvious.
...
Do not read outside `apps/mobile`.
